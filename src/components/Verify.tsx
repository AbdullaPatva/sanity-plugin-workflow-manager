...
export default function Verify(props: VerifyProps) {
  ...
  const documentsWithoutValidMetadataIds = Array.isArray(data) && data.length
    ? data.reduce((acc, cur) => {
  ...
  const documentsWithInvalidUserIds =
    Array.isArray(data) && data.length && Array.isArray(userList) && userList.length
      ? data.reduce((acc, cur) => {
  ...
  const documentsWithoutOrderIds = Array.isArray(data) && data.length
    ? data.reduce((acc, cur) => {
  ...
  const documentsWithDuplicatedOrderIds = Array.isArray(data) && data.length
    ? data.reduce((acc, cur) => {
        const {documentId, orderRank} = cur._metadata ?? {}
  ...
  const removeUsersFromDocuments = React.useCallback(
    async (ids: string[]) => {
      if (!Array.isArray(data) || !Array.isArray(userList)) {
        toast.push({
          title: 'Error',
          description: 'Data or user list not available',
          status: 'error',
        })
        return
      }
